...
${archive || ""}`,
  ...
  /* THE CAST. One correspondent reads the editor's material through their own
     stance and lands wherever that takes them, which is often not where the
     editor landed. Returns JSON so the Cutting Room can grade it later.

     SECURITY is prepended because this op takes untrusted material as input.
     The original snippet omitted it; that omission is the hole an injected
     instruction walks through the first time a correspondent is fed a scraped
     article. */
  cast: {
    maxTokens: 1500,
    search: false,
