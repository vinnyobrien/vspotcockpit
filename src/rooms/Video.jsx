...
import { Check, X, RotateCcw, ExternalLink, Copy } from "lucide-react";
import {import React, { useState, useRef, useCallback } from "react";
...
export default function Video({
  ...
  C, BODY, MONO, DISPLAY, SH, SH_UP, R, Mono, Big, Card, Section, Pill,
  Note, Empty, Problem, Chips, iso, DAYS, parseJSON,
} from "../lib/ui.jsx";
import { callOp, sSet } from "../api.js";
...
/* ============================================================
   ...
   Live: op "clips" (now filtered and rotated server-side by
   _selector.js), and /api/opus-publish through publishClipDirect.

   Twelve binary decisions a day is a card stack, not a checklist.
   Swipe right to keep, left to skip. Nothing publishes on a swipe —
   keeping queues it, and the ship button is a separate, deliberate act.
   ============================================================ */
  ...
  return (
    <>
      <div style={{ position: "relative", height: 340, marginBottom: 18 }}>
        {[clips[n + 1], clips[n]].filter(Boolean).map((c, idx) => {
          const top = idx === 1 || !clips[n + 1];
          const tint = TINTS[(n + idx) % TINTS.length];
          return (
            <div key={c.clipId}
              onPointerDown={top ? down : undefined} onPointerMove={top ? move : undefined}
              onPointerUp={top ? up : undefined} onPointerCancel={top ? up : undefined}
              style={{
                position: "absolute", inset: 0, background: tint, borderRadius: R, padding: 22,
                boxShadow: top ? SH_UP : SH,
                transform: top ? `translateX(${off}px) rotate(${off / 22}deg)` : "scale(.93) translateY(15px)",
                transition: start.current === null ? "transform .24s ease, opacity .24s ease" : "none",
                opacity: fly ? 0 : 1, touchAction: "pan-y", cursor: top ? "grab" : "default",
                display: "flex", flexDirection: "column", overflow: "hidden",
              ...
              <div style={{ marginTop: "auto", paddingTop: 10 }}>
                <Mono s={9}>Right to keep · left to skip</Mono>
              </div>
              {top && (
                <>
                  <Tag show={dx > 40} text="KEEP" col={C.red} side="left" />
  ...
  return (
    <div>
      ...
      {tab === "shorts" && (
        <>
          <Note>
            Filtered against everything already proposed and rotated across themes, so today's are not
            yesterday's. Keeping queues a clip. Publishing is a separate tap.
          </Note>
          ...
          {queue && (
            <>
              <Card tint={C.mint} style={{ textAlign: "center", marginBottom: 16 }}>
                <Big s={40}>{queue.length} KEPT</Big>
                <p style={{ fontSize: 13.5, color: C.ink2, marginTop: 8 }}>
                  Nothing has gone out. Publish each one where you want it.
                </p>
              </Card>
              {queue.map((c) => (
                <Card key={c.clipId} pad={18} style={{ marginBottom: 10 }}>
                  <div style={{ fontFamily: DISPLAY, fontSize: 19, fontWeight: 800, lineHeight: 1.1, color: C.ink }}>{c.title}</div>
                  {c.hook && <p style={{ fontSize: 13, color: C.ink2, marginTop: 5, lineHeight: 1.45 }}>{c.hook}</p>}
                  <div className="flex gap-2 flex-wrap" style={{ marginTop: 12 }}>
                    {ACCOUNTS.map((a) => {
                      const mine = busy === "pub" + c.clipId + a.id;
                      const shipped = published.find((p) => p.title === c.title && p.platform === a.platform);
                      return (
                        <Pill key={a.id} sm tone="ghost" disabled={!!busy || !!shipped}
                          onClick={() => onPublish(c, a, null)}>
                          {mine ? "Posting…" : shipped ? `${a.short} ✓` : `Publish ${a.short}`}
                        </Pill>
                      );
                    })}
                  </div>
      ...
      {tab === "cal" && (
        <>
          ...
          {days.map((dk, i) => {
            ...
            return (
              <Card key={dk} pad={16} accent={full ? C.ink : i === 0 ? C.red : undefined} style={{ marginBottom: 9 }}>
                <Mono c={i === 0 ? C.red : C.ink2} s={9.5}>
                  {DAYS[d.getDay()].slice(0, 2)} {d.getDate()}
                </Mono>
                <div style={{ display: "grid", gap: 7, marginTop: 9 }}>
                  {[["written", "written"], ["video", "video"]].map(([slot, label]) => (
                    <input key={slot} defaultValue={row[slot] || ""} placeholder={`+ ${label}`}
